export const projects = [
  {
    name: "POS Inventory System",
    slug: "pos-inventory-system",
    role: "Backend Developer",
    description: [
      "A full SaaS Point of Sale and Inventory Management system built to help small and medium businesses run daily operations without confusion or manual errors.",
      "The system allows businesses to create accounts and manage multiple stores from one dashboard. Each store can track sales, purchases, inventory levels, and transaction history in real time.",
      "It is designed for business owners who want visibility and control over their operations without relying on spreadsheets or paper records."
    ],
    technologyUsed: [
      "React.js",
      "Express.js",
      "MySQL",
      "Tailwind CSS",
      "REST APIs",
      "Authentication & Multi-business Logic"
    ],
    link: "https://pos-inventory-green.vercel.app/",
    github: "",
    img: "pos.png",
    bgColor: "bg-blue-400"
  },

  {
    name: "Hospital Management System",
    slug: "hospital-management-system",
    role: "Backend Developer",
    description: [
      "A comprehensive Hospital Management System developed for Federal Polytechnic Ilaro to streamline medical and administrative operations.",
      "The platform manages patient records, appointments, medical histories, staff schedules, and billing seamlessly.",
      "Designed to improve operational efficiency, reduce manual errors, and provide real-time insights for healthcare administrators."
    ],
    technologyUsed: [
      "Laravel",
      "Vue.js",
      "MySQL",
      "REST APIs",
      "Authentication & Role-Based Access Control",
      "Patient & Staff Management Logic"
    ],
    link: "https://fpi-clinic-ui.vercel.app/",
    github: "",
    img: "clinic.png",
    bgColor: "bg-red-500"
  },

  {
    name: "Pharmacy Management System",
    slug: "drug-management-system",
    role: "Backend Developer",
    description: [
      "A complete pharmacy management system designed to handle inventory, sales, supplier coordination, and customer purchases in one platform.",
      "The system includes role-based dashboards for admins, suppliers, and customers.",
      "It reduces manual tracking and improves accountability across all roles."
    ],
    technologyUsed: [
      "Node.js",
      "Express.js",
      "TypeScript",
      "MySQL",
      "REST APIs",
      "Authentication & Role-Based Access Control",
      "POS Business Logic"
    ],
    link: "https://zamfara-pharmacy-api.vercel.app",
    github: "",
    img: "pharamacy.png",
    bgColor: "bg-purple-400"
  },

  {
    name: "Nirvana Initiative NGO Website",
    slug: "nirvana-initiative",
    role: "Backend Developer",
    description: [
      "A professional NGO website built to establish trust, communicate impact clearly, and encourage donations.",
      "The platform presents the organization’s mission, programs, and achievements in a clear and accessible way.",
      "Secure online donations are integrated to make supporting the organization simple and reliable."
    ],
    technologyUsed: [
      "Next.js",
      "TypeScript",
      "Tailwind CSS",
      "Express.js",
      "Paystack Payment Gateway"
    ],
    link: "https://www.nirvanainitiative.org/",
    github: "",
    img: "nirvana.png",
    bgColor: "bg-orange-400"
  },

  {
    name: "Nigeria Central – Europe Chamber of Commerce",
    slug: "ncecc-trade",
    role: "Fullstack Developer",
    description: [
      "A corporate trade platform connecting Nigerian businesses with partners across Central Europe.",
      "The platform supports trade facilitation and cross-border collaboration.",
      "It provides a strong digital presence for international partnerships."
    ],
    technologyUsed: [
      "React.js",
      "Express.js",
      "MySQL",
      "Tailwind CSS",
      "REST APIs"
    ],
    link: "https://www.ncecctrade.com/",
    github: "",
    img: "ncecc.png",
    bgColor: "bg-red-400"
  },

  {
    name: "Regional Trade Institute",
    slug: "regional-trade-institute",
    role: "Backend Developer",
    description: [
      "An education and trade platform connecting Africa and Central Europe.",
      "Users can browse programs, add courses to cart, and pay online.",
      "The system supports growth and future integrations."
    ],
    technologyUsed: [
      "React.js",
      "Express.js",
      "MySQL",
      "Tailwind CSS",
      "Paystack Payment Integration"
    ],
    link: "https://www.regionaltradeinstitute.com/",
    github: "",
    img: "rti.png",
    bgColor: "bg-indigo-400"
  },

  {
    name: "CMS Dashboard",
    slug: "cms-dashboard",
    role: "Backend Developer",
    description: [
      "A centralized CMS for managing multiple websites from one dashboard.",
      "Admins can update content without developer support.",
      "Used across multiple production platforms."
    ],
    technologyUsed: [
      "React.js",
      "Express.js",
      "MySQL",
      "Tailwind CSS",
      "Role-Based Access Control"
    ],
    link: "",
    github: "",
    img: "cms.png",
    bgColor: "bg-pink-400"
  },

  {
    name: "Assessment & Recruitment Portal",
    slug: "assessment-recruitment-portal",
    role: "Fullstack Developer",
    description: [
      "An internal assessment system used by organizations to evaluate potential employees during recruitment.",
      "Supports multiple question types including objective, MCQ, true or false, and theory questions where candidates submit written answers.",
      "Tests can be created per job role so all applicants for that role take the same assessment.",
      "Admins can also schedule and assign timed tests to specific individuals, with access restricted to the assigned time window."
    ],
    technologyUsed: [
      "Blade",
      "Tailwindcss",
      "Vanilla Javascript",
      "Laravel",
      "Authentication & Authorization",
      "Role-Based Access Control",
      "Timed Test Scheduling Logic"
    ],
    link: "",
    github: "https://github.com/Niel22/Internal-Assessment-Portal.git",
    img: "assesment.png",
    bgColor: "bg-emerald-400"
  },

  {
    name: "Shoes E-Commerce Platform",
    slug: "shoes-ecommerce",
    role: "Fullstack Developer",
    description: [
      "A complete e-commerce platform for selling shoes online.",
      "Supports product browsing, cart, checkout, and user accounts.",
      "Built with core e-commerce business logic."
    ],
    technologyUsed: [
      "Laravel",
      "Livewire",
      "MySQL",
      "Bootstrap CSS",
      "E-commerce Business Logic"
    ],
    link: "https://womarkbuy.com/",
    github: "https://github.com/Niel22/shoe-website",
    img: "womarkbuy.png",
    bgColor: "bg-indigo-500"
  },

  {
    name: "Blog Platform with RBAC",
    slug: "blog-platform",
    role: "Fullstack Developer",
    description: [
      "A blog platform with structured publishing workflows.",
      "Admins manage roles, authors create content, editors review posts.",
      "Ensures content quality and accountability."
    ],
    technologyUsed: [
      "Laravel",
      "Livewire",
      "MySQL",
      "Bootstrap CSS",
      "Role-Based Access Control"
    ],
    link: "https://blog.womarkbuy.com/",
    github: "https://github.com/Niel22/blog",
    img: "blog.png",
    bgColor: "bg-green-400"
  },
];



export const experiences = [
  {
    period: "January 2025 – Present",
    role: "Software Developer",
    company: "Freelancer",
    location: "Lagos, Nigeria",
    points: [
      "Developed efficient and scalable full-stack applications, from frontend to backend.",
      "Integrated CI/CD pipelines to automate deployments and ensure reliable releases.",
      "Implemented payment gateways and other third-party APIs to enhance app functionality.",
      "Built responsive, user-friendly interfaces and optimized backend services for performance.",
      "Worked on both client-side and server-side solutions for diverse projects."
    ],
  },
  {
    period: "April 2025 – December 2025",
    role: "Backend Developer",
    company: "Zealarax Technology",
    location: "Lagos State, Nigeria",
    points: [
      "Worked within the team to build and scale backend systems for enterprise applications.",
      "Developed REST APIs for a SaaS POS system and CMS using Express.js.",
      "Conducted and participated in code reviews to maintain clean architecture and standards.",
      "Collaborated with frontend developers to integrate APIs with React.js interfaces.",
      "Worked closely with clients alongside the team to gather requirements and deliver solutions.",
      "Contributed to CI/CD pipelines using GitHub Actions to automate deployments."
    ],
  },
  {
    period: "July 2025 – August 2025",
    role: "Junior Web Developer",
    company: "GN128 Solutions",
    location: "Lagos State, Nigeria",
    points: [
      "Single-handedly developed fullstack solutions for the company.",
      "Built and deployed complete web applications from scratch.",
      "Handled both frontend and backend logic using modern frameworks.",
      "Integrated databases and APIs for smooth functionality.",
      "Ensured applications were scalable, maintainable, and performant.",
      "Worked closely with clients to gather requirements and deliver end-to-end solutions."
    ],
  },
  {
    period: "November 2024 – March 2025",
    role: "Backend Development Intern",
    company: "Zealarax Technology",
    location: "Lagos State, Nigeria",
    points: [
      "Contributed to a Hospital Management System project using Laravel.",
      "Developed APIs for inter-department data communication under team guidance.",
      "Worked collaboratively to ensure backend scalability and reliability."
    ],
  },
  {
    period: "April 2025 – May 2025",
    role: "Software Development Intern",
    company: "Prodigy School Suite",
    location: "Lagos State, Nigeria",
    points: [
      "Assisted in developing school management software using PHP.",
      "Integrated INFOBIP API for WhatsApp messaging automation.",
      "Built internal tools using PHP, jQuery, and SQL alongside other developers.",
      "Supported third-party service integrations under team supervision."
    ],
  },
  {
    period: "March 2025",
    role: "Backend Development Intern",
    company: "Ule Homes",
    location: "Lagos State, Nigeria",
    points: [
      "Worked as part of the backend team developing services with TypeScript and raw SQL.",
      "Optimized database queries to improve performance.",
      "Assisted in creating internal APIs to streamline business operations.",
      "Debugged backend issues collaboratively to maintain application stability."
    ],
  },
  {
    period: "March 2023 – 2024",
    role: "Software Development Trainee",
    company: "Kudastech",
    location: "Ogun State, Nigeria",
    points: [
      "Learned and applied HTML, CSS, JavaScript, and Laravel in a guided training environment.",
      "Built an e-commerce and blogging platform as part of training projects.",
      "Practiced clean code and system design with mentor supervision.",
      "Worked with MySQL databases under mentorship."
    ],
  },
];